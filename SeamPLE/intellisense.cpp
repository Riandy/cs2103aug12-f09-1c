...
bool Intellisense::getPriority(vector<string>& tokens)
{
    for(vector<string>::iterator it=tokens.begin();it!=tokens.end();++it)
    {
        ...
        if(checkString.compare(string("HIGH"))==0)
        {
            it=tokens.erase(it);
            return true;
        }
...
{
    action event;
    event.Command=getCommand(tokens,"ADD");
    event.Priority=getPriority(tokens);
    event.Date=getDate(tokens);
    event.EventName=getEventName(tokens);
...
{
    action event;
    event.Command=getCommand(tokens,"DELETE");
    event.EventName=getEventName(tokens);
...
{
    action event;
    event.Command=getCommand(tokens,"EXIT");
...
{
    action event;
    event.Command=getCommand(tokens,"DISPLAY");
    event.Date=getDate(tokens);
...
{
    action event;
    event.Command=getCommand(tokens,"MARK");
    event.Date=getDate(tokens);
    event.EventName=getEventName(tokens);
...
{
    action event;
    event.Command=getCommand(tokens,"INVALID");
...
{
    action event;
    event.Command=getCommand(tokens,"SORT");
...
{
    action event;
    event.Command=getCommand(tokens,"FIND");
    event.Priority=getPriority(tokens);
    event.Date=getDate(tokens);
    event.EventName=getEventName(tokens);
...
{
    action event;
    event.Command=getCommand(tokens,"EDIT");
    event.Priority=getPriority(tokens);
    event.Date=getDate(tokens);
    event.EventName=getEventName(tokens);
